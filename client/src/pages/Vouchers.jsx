...
const Vouchers = () => {
  ...
  // Filter hanya voucher yang AKTIF dan BELUM KEDALUWARSA
  const activeVouchers = useMemo(() => {
    ...
    return vouchers.filter(v => {
      ...
      // Pastikan status aktif & tanggal belum lewat
      return v.is_active && expiryDate > now;
    }).sort((a, b) => {
      // Urutkan berdasarkan created_at (terbaru) atau ID
      return new Date(b.created_at || b.id) - new Date(a.created_at || a.id);
    });
  }, [vouchers]);

  if (loading) return <div className="py-20 text-center text-gray-500">Memuat voucher...</div>;
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 border-b border-border-main pb-6">
          <Link to="/product" className="inline-flex items-center text-text-muted hover:text-theme-primary mb-4 transition-colors">
            <ArrowLeft size={18} className="mr-2" /> Kembali ke Produk
          </Link>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-text-muted mt-2">
                Gunakan kode voucher di bawah ini saat checkout untuk mendapatkan potongan harga.
              </p>
            </div>
          </div>
        </div>
        ...
        {activeVouchers.length > 0 ? (
          ...
          <div className="text-center py-20 bg-white border border-dashed border-border-main rounded-xl">
            ...
            <h3 className="text-xl font-medium text-text-main">Tidak ada voucher aktif</h3>
            <p className="text-text-muted mt-1">
              Saat ini belum ada promo yang tersedia. Cek lagi nanti ya!
            </p>
          </div>
        )}

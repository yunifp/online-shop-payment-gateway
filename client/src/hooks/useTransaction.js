...
export const useTransaction = () => {
  ...
  // [BARU DITAMBAHKAN] Fungsi ambil detail spesifik
  const getTransactionDetail = useCallback(
    async (id) => {
      ...
      try {
        ...
        const message = err.response?.data?.meta?.message || err.message;
        // Opsional: toast.error di sini atau di komponen
  ...
  const repayTransaction = async (transactionId) => {
    try {
      ...
      const res = await axios.post(
        `${API_URL}/transactions/${transactionId}/repay`,
        {},
        {
          withCredentials: true,
        }
  ...
  const getDashboardStats = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API_URL}/transactions/dashboard-stats`, {
        withCredentials: true,
      });
      return res.data.data;
    } catch (err) {
  ...
  return {
    ...
    fetchTransactions,
    fetchMyTransactions,
    getTransactionDetail, // [PENTING] Jangan lupa export ini
    createTransaction,

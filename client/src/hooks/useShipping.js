...
export default function useShipping() {
  ...
  const getProvinces = useCallback(async () => {
    ...
    } catch (error) { toast.error("Gagal memuat provinsi"); }
  }, [API_URL]);
  ...
  const getCities = useCallback(async (provinceId) => {
    ...
    } catch (error) { toast.error("Gagal memuat kota"); } finally { setLoading(false); }
  }, [API_URL]);
  ...
  const calculateCost = async () => {
    try {
      ...
      const options = Array.isArray(responseData) ? responseData : (responseData?.data || []);
      ...
      const msg = error.response?.data?.meta?.message || "Gagal menghitung ongkir.";
      toast.error(msg);
      setShippingCosts([]);
    } finally {
      setLoading(false);

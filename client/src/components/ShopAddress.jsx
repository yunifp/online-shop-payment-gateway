...
const ShopAddress = ({ isOpen, onClose, initialData, onSubmit, loading }) => {
  ...
  const [formData, setFormData] = useState({
    shop_name: '', // Nama Toko / Label Alamat
    street: '',
    details: '',
    postal_code: '',
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (initialData) {
        ...
        setFormData({
            shop_name: '', street: '', details: '', postal_code: '',
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 gap-4">
             <div>
               ...
               <input type="text" required className="w-full border border-gray-300 rounded-lg px-3 py-2.5 outline-none focus:ring-2 focus:ring-theme-primary" placeholder="Pusat, Cabang Bandung, dll" value={formData.shop_name} onChange={e => setFormData({...formData, label: e.target.value})} />
             </div>
          </div>
          ...
          <div className="pt-4 flex justify-end gap-3 border-t border-gray-100">
             <button type="button" onClick={onClose} className="px-5 py-2.5 rounded-lg border border-gray-300 hover:bg-gray-50 text-gray-700 font-medium">Cancel</button>
             <button type="submit" disabled={loading} className="px-5 py-2.5 rounded-lg bg-theme-primary text-white hover:bg-theme-primary-dark font-medium shadow-md flex items-center">
                {loading ? 'Saving...' : <><Save size={18} className="mr-2" /> Save Address</>}
             </button>
          </div>
        </form>
      </div>
